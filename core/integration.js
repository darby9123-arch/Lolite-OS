...
(()=>{
  ...
  window.page=function(id){
    ...
    if(id==='games'){
      ...
      let html=base+`<div class="card" style="margin-top:18px"><h3>🌐 HTML Games</h3><p class="muted">Games from the repository's <b>html games</b> folder.</p><div class="html-game-grid">${repositoryGames.map(g=>`<div class="card html-game-card"><h3>🎮 ${g.name}</h3><span class="html-game-badge">HTML game</span><p>Launch this game inside a Lolite OS window.</p><button class="primary" onclick="launchHTMLGame('${g.name}','${g.file}')">Play</button></div>`).join('')}</div></div>`;
  ...
  window.launchHTMLGame=function(name,file){
    ...
    if(w){w.innerHTML=`<iframe title="${name}" src="html%20games/${encodeURIComponent(file)}" style="width:100%;height:100%;min-height:560px;border:0;border-radius:12px;background:#000"></iframe>`}
  };
